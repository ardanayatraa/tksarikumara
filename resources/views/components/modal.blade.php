...
<div x-data="{ show: @entangle($attributes->wire('model')) }" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-show="show"
    ...
    <div x-show="show" class="fixed inset-0 transform transition-all z-[9998]" x-on:click="show = false"
    ...
    <div x-show="show"
        class="relative mb-6 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full {{ $maxWidth }} sm:mx-auto z-[9999]"
